...
function queryData(){
    startQuery();
    var member = getQueryParam();//java实体类相对应
    var pager = getPager();
    dwrFrontService.listFroMember(member,pager,queryCallback);
}
...
function dblCallback(obj){
    var box = new Sys.msgbox("明细查看","<%=contextPath%>/erp/system_front/member_detail.jsp?pk="+obj.value);
    box.show();
}
function edit(pk){
    openMDITab("<%=contextPath%>/erp/system_front/member_add.jsp?pk="+pk+"&tab="+getMDITab());
}
...
function del(pk){
    confirmmsg("确定要删除用户吗?","delok('"+pk+"')");
}
function delok(pk){
    var pks = new Array();
    pks[0] = pk;
    dwrFrontService.deleteFroMemberByPks(pks,delCallback);
}
...
function delbatch(){
    if(getAllRecordArray() != false){
        confirmmsg("确定要删除用户吗?","delbatchok()");
    }else{
        alertmsg("请选择要删除的用户...");
    }
}
...
function add(){
    openMDITab("<%=contextPath%>/erp/system_front/member_add.jsp?tab="+getMDITab());
}
</script>
</head>
<body>
<%
SysGrid grid = new SysGrid(request,"用户列表");
//放入按钮
...
sccList.add(new SysColumnControl("username","用户名",1,2,2,0));
sccList.add(new SysColumnControl("password","密码",1,2,2,0));
